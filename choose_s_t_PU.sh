...
for combination in $combinations; do
    ...
    for st_combination in "${param_combinations[@]}"; do
        ...
        # 运行 Python 脚本并传递参数
        python tar_adaptation_RES_PU.py $param_str --s="$s" --t="$t"

        # 检查 Python 脚本是否成功运行
        if [ $? -ne 0 ]; then
            echo "$param_str --s=$s --t=$t" >> failed_runs_PU.log
        fi
    done
done
